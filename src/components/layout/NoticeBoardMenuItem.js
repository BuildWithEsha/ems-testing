...
const NoticeBoardMenuItem = ({ currentView, onViewChange }) => {
  const { user } = useAuth();
  ...
  const fetchUnreadCount = async () => {
    try {
      const response = await fetch(`/api/notices/unread-count?user_id=${user.id}&user_role=${user.role || user.user_role || 'employee'}`);
      if (response.ok) {
  ...
  const isActive = currentView === 'noticeBoard';
...
export default NoticeBoardMenuItem;
